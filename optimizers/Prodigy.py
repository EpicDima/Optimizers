...
class Prodigy(Optimizer):
    ...
    param_descriptions = {
        "lr": "безразмерный множитель поверх автоматически подбираемого масштаба d, обычно 1.0",
        "beta1": "коэффициент EMA первого момента (сглаживание направления градиента)",
        "beta2": "коэффициент EMA второго момента; sqrt(beta2) также сглаживает счётчики r и s D-адаптации",
        "d0": "стартовое значение масштаба d — оценки расстояния до минимума; дальше d только растёт",
        "eps": "малая добавка в знаменатель (умножается на d) для численной устойчивости",
    }
