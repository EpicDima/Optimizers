...
class MARS(Optimizer):
    ...
    param_descriptions = {
        ...
        "gamma": "сила вариационной поправки; здесь, при точном градиенте, действует как экстраполяция градиента",
        "beta1": "коэффициент EMA первого момента (по скорректированному градиенту c); входит и в масштаб поправки",
        "beta2": "коэффициент EMA второго момента (квадратов скорректированного градиента)",
        "eps": "малая добавка в знаменатель для численной устойчивости, настраивать обычно не нужно",
        "weight_decay": "затухание весов — тянет точку к началу координат, искажая минимизируемую функцию",
    }
