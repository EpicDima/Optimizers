...
class Adafactor(Optimizer):
    ...
    param_descriptions = {
        "lr": "верхняя граница относительного шага rho_t = min(lr, 1/sqrt(t))",
        "beta2_decay": "показатель затухания второго момента: вес свежего квадрата градиента равен t^beta2_decay",
        "eps1": "нижний порог оценки второго момента — численная защита знаменателя",
        "eps2": "нижний порог масштаба параметра: не даёт шагу вырождаться около начала координат",
        "d": "порог клиппинга: обновление с RMS выше d пропорционально урезается",
