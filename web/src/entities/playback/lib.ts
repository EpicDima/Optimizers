...
/** Ручной rAF-цикл воспроизведения — сознательно НЕ используем Plotly frames/animate
 * API (не годится для произвольного хвостового окна относительно текущего кадра,
 * плохо сочетается с декларативной моделью react-plotly.js). Вместо этого просто
 * тикаем `frame` в сторе, а PlotPanel уже сам перерисовывает данные через Plotly.react.
 *
 * В отличие от matplotlib.animation (фиксированный таймер 30мс + frames_per_tick для
 * пропуска кадров), rAF даёт произвольную частоту тиков, поэтому кадры продвигаются
