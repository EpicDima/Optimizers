...
class Function:
    def __init__(self) -> None:
        ...
        self.standard_functions = {
            ...
            # методы с крупным шагом перелетают в бассейн глобального минимума
            # (-1.0316), а адаптивная Adam-семья оседает в локальном (-0.2155);
            # из прежнего старта (-1.7, 1.2) глобальный не находил никто
            "Функция шестигорбого верблюда": StandardFunction(
                "4*x^2 - 2.1*x^4 + x^6/3 + x*y - 4*y^2 + 4*y^4", (-2, 2, -1.5, 1.5), (-1.1, 1.0)
            ),
